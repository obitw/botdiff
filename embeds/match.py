...
async def build_match_embed(
    ...
    try:
        ...
        for idx, tp in enumerate(tracked_players):
            ...
            streak_text = (
                f"🥶 Loose streak: {-streak}\n"
                if streak <= -3
                else (f"🔥 Win streak: {streak}\n" if streak >= 3 else "")
            )
            ...
            embed.set_thumbnail(
                url=DDRAGON_CHAMPION_ICON.format(version=version, champion=champion)
            )
    ...
    def __init__(
        ...
        for tp in tracked_players:
            self.add_item(
                discord.ui.Button(
                    label="DPM.LOL",
                    url=DPM_PROFILE_URL.format(
                        riot_id=tp["riot_id"].replace(" ", ""), tag=tp["tag"]
                    ),
